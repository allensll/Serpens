...
def main():
    parser = argparse.ArgumentParser(description='Train ResNet32 for CIFAR100 in PyTorch')
        ...
        if is_best:
            ...
            if args.save_model:
                torch.save({
                    ...
                    'best_acc': best_acc,
                }, '../pretrained/resnet32/checkpoint_best.pth')

        if args.save_model:
            torch.save({
                'state_dict': model.state_dict(),
                'best_acc': best_acc,
            }, '../pretrained/resnet32/model.pth')
...
def acc():
    ...
    train_loader, test_loader = utils.load_CIFAR100(args.batch_size, test_batch_size=test_batch_size, **kwargs)
    model = resnet32(num_classes=100, pretrained=True)
    model = model.to(device)
    model.eval()
    cudnn.benchmark = True
